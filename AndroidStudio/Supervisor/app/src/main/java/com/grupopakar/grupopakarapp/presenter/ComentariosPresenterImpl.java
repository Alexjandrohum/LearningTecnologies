...
public class ComentariosPresenterImpl implements ComentariosContract.ComentariosPresenter, ComentariosContract.ComentariosDAO.OnFinishedListener {
    ...
    @Override
    public void cargaImagen(String path, String nombre) {
        if (view != null){
            view.updateProgress(false);
            dao.cargaImagen(path, nombre, this);
        }
    }
    ...
    @Override
    public void onFinishedImagen(String respuesta) {
        if (view != null){
            view.updateProgress(true);
            view.muestraMensajeImagen(respuesta);
        }
    }
}
